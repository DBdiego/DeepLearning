...
class CNN:
    ...
    def __init__(self, network_index, gpu_index, trainset, testset,
        ...
        # --------------------------------------
        # Parameters:
        MAXTRAINTIME = 10  # seconds, not sure if this is a good time. Note that testing time is not included, this is (often) slightly less than 1 epoch time.
        BATCH_SIZE = 10
        LR = 0.001
        MOMENTUM = 0.9
        ...
        while traintime < MAXTRAINTIME:
            ...
            for i, data in enumerate(trainloader, 0):  # for every batch, start at 0
                ...
                if i % 20 == 19:  # print every 200 mini-batches
                    print(f'\t --> [{epoch}, {i + 1}] loss: {np.round(running_loss / 20,2)}')
                    #print('\t',losslst[-5:])
